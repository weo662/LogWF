...
result_file_name = "_experiment_results.csv"
model_file_name = "_checkpoint.pt"
...
def arg_parse():
    ...
    parser.add_argument('--model', default="LogWF", help='The model to be used')
...
def record_result(file_path, data_setting, result_dict):
    with open(file_path, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        if csvfile.tell() == 0:
            writer.writerow(data_setting.keys())
        writer.writerow(data_setting.values())
...
def gather_results(result_dict, one_round_result):
